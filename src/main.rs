use piston_window::{PistonWindow, G2dTextureContext, G2dTexture, Context, G2d, GfxDevice, EventLoop, WindowSettings, CharacterCache};
use piston_window::{PressEvent, UpdateEvent, RenderEvent};
use piston_window::Button::{self};
use piston_window::{Key, Texture, Flip, TextureSettings, Transformed};
use piston_window::{MouseButton, MouseCursorEvent};
use piston_window::{rectangle, text};
use piston_window::Glyphs::{self};
...
const FULL_WHITE: [f32; 4] = [1.0, 1.0, 1.0, 1.0];
const FULL_BLACK: [f32; 4] = [0.0, 0.0, 0.0, 1.0];
...
fn main() {
    let mut window: PistonWindow = WindowSettings::new("Snake", [WINDOW_SIZE, WINDOW_SIZE]).exit_on_esc(true).build().unwrap();
    window.set_ups(6);
    ...
    let mut snake = Snake::new(WINDOW_SIZE);
...
fn draw_game_over(context: Context, graphics: &mut G2d, glyphs: &mut Glyphs, device: &mut GfxDevice) {
    ...
    let game_over_text_style = text::Text::new_color(FULL_WHITE, 32);
    ...
    let restart_button_text_style = text::Text::new_color(FULL_WHITE, BUTTON_TEXT_SIZE);
